...
public class CountingOfTime : MonoBehaviour {
	...
	private float speed = 360.0f / (24.0f*60.0f); //one hour in game = 15s in real life
	private float time;
	...
	public void getDate(out int d, out int m, out int y) {
		d = day;
		m = month;
		y = year;
	}
...
//		}
		...
		if(sunHeight >= astroHeight) {
			sun.light.intensity = 1.0f * (sunHeight/(astroHeight*4.0f));
			...
			moon.light.intensity = 0.1f * (sunHeight/(astroHeight*4.0f));
